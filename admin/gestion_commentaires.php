...
if($_POST){ // equivalent a !empty($_POST), qui signifie que le formulaire a été envoyé
    ...
    if(empty($contenu)){
        ...
        $requete = executeRequete("REPLACE INTO commentaire VALUES(:id_commentaire, :commentaire, :membre_id, :id_annonce, NOW())", array(
...
$resultat = executeRequete("SELECT c.id_commentaire, m.pseudo, m.email, a.id_annonce, a.titre, c.commentaire, c.date_enregistrement FROM commentaire c INNER JOIN membre m ON c.membre_id = m.id_membre INNER JOIN annonce a ON c.annonce_id = a.id_annonce"); // on selectionne tout les produits
...
while ($produit = $resultat->fetch(PDO::FETCH_ASSOC)) { // produit est un array avec toutes les informations d'un seul produit à chaque tour de boucle. On le parcourt donc avec une boucle foreach :
    $contenu .= '<tr>';
        ...
        $contenu .= '<td>
                            <a href="?action=modifier&id_commentaire='. $produit['id_commentaire'] .'">modifier</a> |
